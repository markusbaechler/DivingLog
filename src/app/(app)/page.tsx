      ...
      {dives.length === 0 ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <StatCard label="Dives" value={stats.totalDives} accent />
            <StatCard label="Time" value={stats.totalBottomTimeH} unit="h" />
            <StatCard label="Deepest" value={stats.maxDepth} unit="m" />
          ...
          {points.length > 0 && (
            <section>
              <div className="mb-2 flex items-center justify-between">
                ...
                <Link
                  href="/map"
                  className="flex items-center gap-1 text-sm text-ocean-600 hover:underline dark:text-ocean-400"
                >
                  View all <ArrowRight size={14} />
                </Link>
              </div>
              <div className="h-64 overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800">
  ...
  return (
    <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center dark:border-slate-700 dark:bg-slate-900">
      ...
      <p className="mx-auto mt-2 max-w-md text-sm text-slate-500 dark:text-slate-400">
        No dives yet. Log your first dive manually or import your data from
        Garmin Connect or DivingLog.
      </p>
