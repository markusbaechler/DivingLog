...
export function DiveCard({ dive }: { dive: DiveWithSite }) {
  return (
    <Link
      href={`/dives/${dive.id}`}
      className="block rounded-xl border border-slate-200 bg-white p-4 transition hover:border-ocean-300 hover:shadow-sm"
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            {dive.dive_number != null && (
              <span className="rounded-md bg-ocean-50 px-2 py-0.5 text-xs font-bold text-ocean-700">
                #{dive.dive_number}
              </span>
            )}
            <h3 className="truncate font-semibold text-slate-800">
              {dive.dive_site?.name ?? "Ohne Tauchplatz"}
            </h3>
          </div>
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 text-sm">
        <Metric icon={<Gauge size={14} />} label="Tiefe">
          {dive.max_depth != null ? `${dive.max_depth} m` : "—"}
        </Metric>
        <Metric icon={<Clock size={14} />} label="Dauer">
